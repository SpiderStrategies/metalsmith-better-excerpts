    ...
    return function(files, metalsmith, done) {
        ...
        _.forEach(files, function(file, filename) {
            ...
            var excerpt = file.excerpt ||
                            ...
                            getExcerptByNParagraphs(file, options.paragraphCount) ||
                             '';
            ...
            file.excerpt = excerpt && excerpt.length > 0 ? excerpt : '';
        });
    };
};
 ...
 * @return {mixed}       excerpt string or false
 */
function getExcerptByNParagraphs(file, count) {
    var $ = cheerio.load(file.contents.toString()),
        p = $('p').slice(0, count),
        excerpt = p.length
                    ? p.map(function (idx, p) {
                            return $(this).html()
                        }).get().join('<br />')
                    : false
